...
| extension  | `packages/extension/ui/state.ts`                                | `options`, `selection`, `selectedNode`, `activePlugin`                                                                   | Deterministic UI storage defaults and reactive selection/plugin state | P2       |
...
- `ui/state.ts`:
  - default panel/storage option initialization behavior.
  - selection-derived `selectedNode` behavior.
  - active plugin resolution behavior for found/missing/null sources.
