...
- Themeable vectors: `themeable=true` means the SVG can safely adopt one contextual color channel. In `smart` mode, that color is typically already evidenced on the emitted `svg` root markup while the inner paths use `currentColor`. It does not mean the SVG exposes multiple independent color parameters.
